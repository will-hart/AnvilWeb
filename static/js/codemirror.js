...
window.CodeMirror = (function() {
  ...
  function makeDisplay(place, docStart) {
    ...
    d.wrapper = elt("div", [d.inputDiv, d.scrollbarH, d.scrollbarV, d.scrollbarFiller, d.scroller], "CodeMirror");
  ...
  function findCachedMeasurement(cm, line) {
    ...
    for (var i = 0; i < cache.length; ++i) {
      var memo = cache[i];
      if (memo.text == line.text && memo.markedSpans == line.markedSpans && cm.display.scroller.clientWidth == memo.width && memo.classes == line.textClass + "|" + line.bgClass + "|" + line.wrapClass) return memo.measure;
    }
  }
  ...
  function coordsChar(cm, x, y) {
    ...
    if (lineNo > last) return PosMaybeOutside(doc.first + doc.size - 1, getLine(doc, last).text.length, true);
    if (x < 0) x = 0;
  ...
  function readInput(cm) {
    ...
    else if (cm.state.overwrite && posEq(from, to) && !cm.state.pasteIncoming) to = Pos(to.line, Math.min(getLine(doc, to.line).text.length, to.ch + (text.length - same)));
  ...
  CodeMirror.prototype = {
    ...
    moveH: operation(null, function(dir, unit) {
      var sel = this.doc.sel,
        pos;
      if (sel.shift || sel.extend || posEq(sel.from, sel.to)) pos = findPosH(this.doc, sel.head, dir, unit, this.options.rtlMoveVisually);
  ...
  function option(name, deflt, handle, notOnInit) {
    ...
    if (handle) optionHandlers[name] = notOnInit ?
    function(cm, val, old) {
  ...
  function lineIsHiddenInner(doc, line, span) {
    ...
    for (var sp, i = 0; i < line.markedSpans.length; ++i) {
      ...
      if (sp.marker.collapsed && sp.from == span.to && (sp.marker.inclusiveLeft || span.marker.inclusiveRight) && lineIsHiddenInner(doc, line, sp)) return true;
    }
  }
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    removeLine: function(line) {
      if (isLine(this, line)) replaceRange(this, "", Pos(line, 0), clipPos(this, Pos(line + 1, 0)));
    },
  ...
  function addToHistory(doc, change, selAfter, opId) {
    ...
    if (cur && (hist.lastOp == opId || hist.lastOrigin == change.origin && change.origin && ((change.origin.charAt(0) == "+" && hist.lastTime > time - 600) || change.origin.charAt(0) == "*"))) {
